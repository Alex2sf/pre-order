<x-admin-layout title="{{ isset($product) ? 'Edit Produk' : 'Tambah Produk' }}" :header="isset($product) ? '✏️ Edit Produk' : '➕ Tambah Produk'" :subtitle="isset($product) ? 'Perbarui detail produk' : 'Tambahkan produk pre-order baru'">
    <div class="max-w-3xl">
        <form method="POST" action="{{ isset($product) ? route('admin.products.update', $product) : route('admin.products.store') }}" 
            ...
            <div class="flex items-center gap-3">
                ...
                <a href="{{ route('admin.products.index') }}" class="px-6 py-3 bg-slate-100 text-slate-600 font-semibold rounded-xl hover:bg-slate-200 transition">Batal</a>
            </div>
        </form>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 mt-6">
            ...
            {{-- Add Variant Form --}}
            <form method="POST" action="{{ route('admin.products.variants.store', $product) }}" class="flex flex-wrap items-end gap-3 mb-4 p-4 rounded-xl bg-slate-50">
                @csrf
