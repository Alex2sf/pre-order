<x-owner-layout title="Produk" :header="'📦 Daftar Produk'" :subtitle="'Kelola produk pre-order Anda'">
    <div class="flex items-center justify-between mb-6">
        <p class="text-sm text-slate-500">Total: {{ $products->total() }} produk</p>
        <a href="{{ route('owner.products.create') }}" class="inline-flex items-center gap-2 px-5 py-2.5 bg-gradient-to-r from-violet-600 to-indigo-600 text-white font-semibold rounded-xl shadow-lg shadow-violet-500/20 hover:shadow-violet-500/40 transition-all hover:-translate-y-0.5 text-sm">
            + Tambah Produk
        </a>
    </div>
        ...
        <div class="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 hover:shadow-md transition">
            <div class="flex items-start justify-between gap-4">
                ...
                <div class="flex items-center gap-2 shrink-0">
                    <form method="POST" action="{{ route('owner.products.toggle', $product) }}">
                        @csrf
                    ...
                    <a href="{{ route('owner.products.edit', $product) }}" class="px-3 py-2 rounded-xl text-xs font-semibold bg-violet-50 text-violet-700 hover:bg-violet-100 transition">✏️ Edit</a>
